...
const Formulario = () => {
  ...
  if (nome.trim() == "" || nome <= 5) {
    mensagem += "Erro: O campo Nome do Cliente está vazio!\n";
  }

  if (data.trim() == "") {
    mensagem += "Erro: O campo Data está vazio!\n";
  }

  if (telefone.trim() == "" || telefone <= 5) {
    mensagem += "Erro: O campo Telefone está vazio!\n";
  }

  if (mail.trim() == "" || mail <= 5) {
    mensagem += "Erro: O campo E-mail está vazio ou menos que 5 caracteres!\n";
  }

  if (produto.trim() == "" || produto < 0) {
    mensagem += "Erro: O campo Produto está vazio ou é Menor do que 0!\n";
  }

  if (qtd.trim() == "" || qtd < 0) {
    mensagem += "Erro: O campo Quantida está vazio ou é Menor do que 0!\n";
  }

  if (valor.trim() == "" || valor < 0) {
    mensagem += "Erro: O campo Valor está vazio ou é Menor do que 0!\n";
  }
...
const calcular = () => {
  ...
  inPercDesc.value = perDesconto.toFixed(2);
